...
针对问题四：基于问题一的模型，构建调头运动模型。首先证明在给定条件下，调头曲线长度为常数，无法进一步减小。随后，将调头路线划分为四个部分，通过几何关系求解出各部分的曲线方程。沿用问题一的推导方法，首先求解龙头前把手位置函数，再通过递推关系和求导计算其余把手的位置函数和速度函数。我们对调头空间附近的递推进行了详细的分类讨论，并给出了相应的递推方程。最终，将极坐标转换为直角坐标，求得各极凳把手在各时刻的位置和速度，详见表5，表6和result4.xlsx。

针对问题五：在调头运动模型的基础上，构建各把手最大速度的优化模型。首先，通过比例转换，将目标转化为在龙头速度仍为 $1\mathrm{m / s}$ 的条件下，求解各把手在所有时刻的最大速度。随后，基于问题四的速度规律及物理规律确定最大速度出现的大致时间范围，接着通过大步长搜索确定了最大速度出现的区间为[14s,15s]。由于函数在该区间内呈单峰性，应用三分搜索算法进一步精确定位最大速度。最后，通过比例关系还原龙头的最大速度，得到龙头最大速度约为 $1.246266\mathrm{m / s}$
...
![](images/368851caacc98f1b09f06a2e39f623d506afd121dc30773eea0065cdb86e88ee.jpg)  
图 2 相邻把手几何关系示意图
...
\- 如图3所示，龙头前把手的位置关于时间均匀分布在螺线上，符合题目中龙头前把手速度恒为 $v_{0} = 1 \, \text{m/s}$ 的条件。

![](images/2708514250d0e92c0a23db97740de52035bae838004ef9d8e10a0e15dd312477.jpg)  
图 3 t = 0s 至 t = 300s 龙头前把手位置  
...
![](images/53ee8b16cb78a8cfe3f4df7ff3266775063c75b1b7ef2f634f1c1ef75996cfef.jpg)  
图 4 不同时刻下板凳龙把手的速度情况

\- 若作出极角的变化图像（如图5所示），我们可以发现越靠内圈的把手，极角变化速度越快。这是因为越靠内圈，把手的运动半径就越小，为了保持相同的速度，极角的变化率就会变快。这也验证了我们建立的模型的合理性。

![](images/6cd4c4dbce84d9f668e3b5947209138d96a01d61a755574ffc7c93a0bdd4fa6a.jpg)  
...
接着我们需要求解内圈编号为 $i$ 的板凳靠近外圈的两个顶点的坐标，设其坐标为 $A_{i}(x_{A_{1}},y_{A_{i}})$ 和 $B_{i}(x_{B_{1}},y_{B_{i}})$ 。为了简化表达，我们通过题目给定的板凳参数，记孔中心与最近的板头距离为 $a = 0.275\mathrm{m}$ ，与板凳上下两边距离为 $b = 0.15\mathrm{m}$ ，如图6所示。

顶点 $(x_{A_i},y_{A_i})$ 存在如图6所示的几何关系。图中，角 $\alpha_{i}$ 满足：

$$
\begin{array}{l} \sin \alpha_ {i} = \frac {- \Delta y}{\sqrt {(\Delta x) ^ {2} + (\Delta y) ^ {2}}} \\ \cos \alpha_ {i} = \frac {- \Delta x}{\sqrt {(\Delta x) ^ {2} + (\Delta y) ^ {2}}} \end{array}
$$

![](images/e102c6a8333f809ebeb3b2003a1a9ec5778ddbab7a7a07fcc41fc8651c9e4f00.jpg)  
图 6 编号为 i 的板凳几何关系示意图

其中 $\Delta x = x_{i + 1} - x_i,\Delta y = y_{i + 1} - y_i$ 。因此通过几何关系，我们可以求出该顶点的坐标：

$$
\begin{array}{l} x _ {A _ {i}} = x _ {i} + a \cos \alpha_ {i} - b \sin \alpha_ {i} \\ y _ {A _ {i}} = y _ {i} + a \sin \alpha_ {i} + b \cos \alpha_ {i} \end{array}\tag{5}
$$

同理可得另一个顶点的坐标：

$$
\begin{array}{l} x _ {B _ {i}} = x _ {i + 1} - a \cos \alpha_ {i} - b \sin \alpha_ {i} \\ y _ {B _ {i}} = y _ {i + 1} - a \sin \alpha_ {1} + b \cos \alpha_ {i} \end{array}\tag{6}
$$

于是我们以编号为 $i$ 的板凳的两个顶点到编号为 $j$ 的板凳前后把手所在直线 $l_{j}$ 的距离 $d_{A}, d_{B}$ 为判定函数。通过点到直线距离公式求出两个函数表达式：

$$
\begin{array}{l} d _ {A} (t, i, j) = \frac {\left| \left(y _ {j + 1} - y _ {j}\right) x _ {A _ {i}} - \left(x _ {j + 1} - x _ {j}\right) y _ {A _ {i}} + x _ {j + 1} y _ {j} - x _ {j} y _ {j + 1} \right|}{\sqrt {\left(x _ {j + 1} - x _ {j}\right) ^ {2} + \left(y _ {j + 1} - y _ {j}\right) ^ {2}}} \\ d _ {B} (t, i, j) = \frac {\left| \left(y _ {j + 1} - y _ {j}\right) x _ {B _ {i}} - \left(x _ {j + 1} - x _ {j}\right) y _ {B _ {i}} + x _ {j + 1} y _ {j} - x _ {j} y _ {j + 1} \right|}{\sqrt {\left(x _ {j + 1} - x _ {j}\right) ^ {2} + \left(y _ {j + 1} - y _ {j}\right) ^ {2}}} \end{array}
$$
...
根据假设，我们只需要在最内圈以及其外侧相邻一圈间，运用碰撞检测模型，就能判断舞龙队是否发生碰撞。首先从龙头开始遍历最内圈的板凳（即模型中编号为 $\phi$ 的板凳），再找到与其相邻一圈离它最近的第一个板凳以及其前后各两张板凳（即模型中编号为 $\phi$ 的板凳），运用碰撞检测公式检查板凳之间的距离。如果任意一对板凳的最小距离小于0.15m，则认为发生了碰撞。
...
![](images/1853e098b326ea9fb52c5c7b765073eb06ecb875250565b7c8ddf6eebe6e17f4.jpg)  
图 8 停止盘入时刻板凳龙碰撞情况

我们还绘制了图9所示图像（红色虚线为 y = 0.15m），描绘了最内圈板凳的外侧顶点距其外一圈板凳中心线的最短距离与时间 t 的关系。

![](images/1ba3f432cb6e6ee8098edf071086fd4322bcbde414708c689c7c05e338a00c9a.jpg)  
...
鉴于每块板凳宽 $0.3\mathrm{m}$ ，且把手中心均位于行进螺线上，因此容易得出螺距 $d$ 的下界： $0.3\mathrm{m}$ 。由问题二可知，当 $d = 0.55\mathrm{m}$ 时，舞龙队会在第412秒左右发生第一次碰撞，此时龙头前把手坐标为(1.209931,1.942784)，显然在圆 $O$ 内。由于该碰撞为 $d = 0.55\mathrm{m}$ 第一次碰撞，说明在舞龙队到达调头范围前并未发生过碰撞，因此可将 $0.55\mathrm{m}$ 设为螺距 $d$ 的上界。即 $d$ 的取值范围为 $[0.30\mathrm{m},0.55\mathrm{m}]$ ：
...
根据在问题二提出的碰撞模型，调整螺距 d，我们可以判断在任意时刻 t，最内圈的任意板凳是否与其相邻一圈的板凳发生碰撞。由问题二可知，第一次碰撞会发生在最内圈与其外圈一层的相邻板凳之间，因此为了保证不发生碰撞，需要保证最内圈任意板凳外侧的前顶点 $A_{i}$ 和后顶点 $B_{i}$ 到其相邻一圈任意板凳的前后把手所在直线 $I_{j}$ 的距离 $d_{A}$ 和 $d_{B}$ 大于安全阈值 0.15m。

$$
\min_{\substack{t\in [0,t_{0}]\\ i,j}}\min \{d_{A}(t,i,j),d_{B}(t,i,j)\} >0.15\mathrm{m}\tag{7}
$$
...
![](images/a2a4ba93b6e42f08c66312be250d50e3cb07598dc5213b95948d2ee5bcf2abb7.jpg)  
图 14 最小螺距下龙头进入调头空间的状态

![](images/f240381512bb7d0b8da2a21e4f3b62087ffcce7c5ddea7c57e3fb321d1d35b00.jpg)  
...
![](images/b2bc824b7868df7b1a66768d7e2719dd128d85c35ab30f4f8adb9a6bb18a4b2a.jpg)  
图16 调头曲线示意图
...
![](images/1e537ecf0b65caa2f5ea0c5804255db9a42c0f4504821deea1b5c13dfa491657.jpg)  
...
$$
\begin{array}{l} O _ {1}: (r _ {\min} \cos \theta_ {\min} - R _ {1} \sin (\theta_ {\min} + \alpha), r _ {\min} \sin \theta_ {\min} + R _ {1} \cos (\theta_ {\min} + \alpha)) \\ O _ {2}: (- r _ {\min} \cos \theta_ {\min} + R _ {2} \sin (\theta_ {\min} + \alpha), - r _ {\min} \sin \theta_ {\min} - R _ {2} \cos (\theta_ {\min} + \alpha)) \end{array}
$$
...
$$
\begin{array}{l} x = x _ {o _ {1}} + R _ {1} \cos \theta_ {i} \\ y = y _ {o _ {1}} + R _ {1} \sin \theta_ {i} \end{array}\tag{8}
$$

第二段弧 $\widehat{DF}$ 参数方程如下：

$$
\begin{array}{l} x = x _ {o _ {2}} - R _ {2} \cos \theta_ {i} \\ y = y _ {o _ {2}} - R _ {2} \sin \theta_ {i} \end{array}\tag{9}
$$
...
![](images/e66c17fb3ce72341d70222aa74786fa5da32b4dbff1ab66f82ee24bbb02ee4ea.jpg)  
图 18 板凳龙调头路径示意图
...
其中， $x = \frac{d}{2\pi}\theta_{n + 1}\cos \theta_{n + 1},y = \frac{d}{2\pi}\theta_{n + 1}\sin \theta_{n + 1},\theta_{n + 1}$ 求解范围为： $[\theta_{\mathrm{min}},\theta_{\mathrm{min}} + \pi ]$ 位于曲线 $M$ 和曲线 $N$
...
其中， $\theta_{n + 1}$ 求解范围为： $[\theta_{\mathrm{min}} - \alpha -\pi /2,\theta_{\mathrm{min}} + \alpha -\pi /2]$
...
\- $t \in [-100\mathrm{s}, 0\mathrm{s}]$ : 此时龙头前把手位于曲线 $P$ 内, 其运动轨迹依然为等距螺线, 与问题一条件相同。因此, 可以直接用模型一中的 (1) 式求解 $\hat{\theta}_0$ 。

\- $t \in [0\mathrm{s}, t_N]$ : 此时龙头前把手位于曲线 $M$ 内, 其运动轨迹为弧 $\widehat{BD}$ , 有 $\dot{\theta}_0 = -v_0 / R_1$ 。

\- $t \in [t_N, t_Q]$ : 此时龙头前把手位于曲线 $N$ 内，其运动轨迹为弧 $\widehat{DF}$ ，有 $\dot{\theta}_0 = v_0 / R_2$ 。

$t \in [t_{Q}, 100s]$ ：此时龙头前把手位于曲线 Q 内，运动轨迹同样为等距螺线，可用模型一中方法求解。但是需要注意，需要将 (1) 式中的 $\theta_{0}$ 替换为 $\theta_{0} + \pi$ ，而且 $\dot{\theta}_{0} > 0$ ，因此在去绝对值的时候不需要添负号。
...
$$
\begin{array}{r l} & A = (x _ {n} - \frac {d}{2 \pi} \theta_ {n + 1} \cos \theta_ {n + 1}) \dot {x} _ {n} + (y _ {n} - \frac {d}{2 \pi} \theta_ {n + 1} \sin \theta_ {n + 1}) \dot {y} _ {n} \\ & B = (x _ {n} - \frac {d}{2 \pi} \theta_ {n + 1} \cos \theta_ {n + 1}) (\cos \theta_ {n + 1} - \theta_ {n + 1} \sin \theta_ {n + 1}) \\ & \quad + (y _ {n} - \frac {d}{2 \pi} \theta_ {n + 1} \sin \theta_ {n + 1}) (\sin \theta_ {n + 1} + \theta_ {n + 1} \cos \theta_ {n + 1}) \\ & \dot {x} _ {n} = v _ {n} \sin \theta_ {n} \\ & \dot {y} _ {n} = - v _ {n} \cos \theta_ {n} \end{array}
$$
...
对于龙身以及龙尾部分，我们仍根据龙头前把手数据递归计算得到相应位置和速度。由于调头路径的复杂性，需要首先判断相邻把手在不同时间点所属路径。再按照位置推导关系中所给出的分类求解方程，在给定求解范围内，逐步更新各把手的角度和坐标。表5和表6是从 $t = -100\mathrm{s}$ 至 $t = 100\mathrm{s}$ 时部分时刻以及部分把手的位置与速度表格。全部把手每秒的位置速度信息见表格文件 result4.xlsx。

![](images/ed5b366b406e1007f0b6abcab369509c041e6d75535bdafcb105189fd896417d.jpg)  
图19 $t = -100\mathrm{s}$ 至 $t = 100\mathrm{s}$ 之间龙头前把手的位置
...
![](images/44d8c42b9172b9f90e096f8ec16d0f682e4a18a475235b34272c3da670147290.jpg)  
...
\- 时间 $t$ 的取值范围：观察问题四得到的各把手的速度数据，不准发现：当 $t \leq 0$ 时，速度的最大值一定出现在龙头，恒为 $\gamma_0$ ；而当 $t > 0$ 时，速度的最大值一开始出现在龙头，后面随着 $t$ 的增大，整体呈现向龙尾方向移动的趋势。而且在 $t = 14\mathrm{s} - 15\mathrm{s}$ 附近（也就是龙头在 $F$ 点附近时），速度的最大值突然出现一个极大值。在这之后，速度的最大值整体呈现缓慢变大的趋势。

因此，我们只需要在龙头在 F 点附近和龙尾在 F 点附近时计算速度的最大值即可（经计算，该时间间隔为 $t = 376s \sim 384s$ ）。

$$
\begin{array}{c} t \in [ 1 3 \mathbf {s}, 1 6 \mathbf {s} ] \cup [ 3 7 6 \mathbf {s}, 3 8 4 \mathbf {s} ] \\ n \in \{0, 1, 2 2 3 \} \end{array}
$$

因此，我们确定所有把手速度最大值的单目标优化模型为

$$
\begin{array}{l} \max _ {t, n} v _ {n} (t) \\ \text { s   .   t   . } \quad \left\{ \begin{array}{l} t \in [ 1 3 \mathrm{s}, 1 6 \mathrm{s} ] \cup [ 3 7 6 \mathrm{s}, 3 8 4 \mathrm{s} ], \\ n \in \{0, 1, 2 2 3 \}. \end{array} \right. \end{array}
$$
...
![](images/9524d3028abca53eb2c4b4834e4f717360297c48ee3e48c7204824616e36be18.jpg)  
图 21 三分搜索法示意图
...
分别计算在这两个时间点上的所有把手速度的最大值 $\max_{n}v_{n}(t_{1})$ 和 $\max_{n}v_{n}(t_{2})$ 。Step3：比较中间点的函数值：
...
![](images/72c4ea388cd080ec88065b434ead892b139b165a0fd0f6c82b92bf2c36b6d8cf.jpg)  
图22 $t = -100\mathrm{s}$ 至 $t = 500\mathrm{s}$ 把手速度最大值

![](images/7eac785f156db992364f644f23f8577791668fdb50756f529ba32780cb8bd9e9.jpg)  
图23 $t = 14\mathrm{s}$ 至 $t = 15\mathrm{s}$ 把手速度最大值
...
![](images/10228d955f6e33421786ae71adf8a099781094d851c6e7e98b8eae1308055286.jpg)
...
hole_dia = 0.275 # 孔离板头距离(m)  
hole_dia = 0.055 # 孔的直径(m)  
dra_len = 223 # 板凳个数  
font = FontProperties(fnace*/System/Library/Fonts/Supplemental/Times New Roman.ttf*)
```

```python
font2 = FontProperties(fnase="/Systes/Library/Fonts/Supplemental/Songti.ttc")

class Dragon():
    def __init__(self, d=0.55, v0=1, theta0=32 * pi): # d为螺距(m), v0为龙头前把手速度(n/s)
    self.d = d # 螺距(m)
    self.v0 = v0 # 龙头前把手速度(n/s)
...
dis1 = abs(delta_yj) * x1 - delta_xj * y1 + self.pos[j + i][0] * self.pos[j][1]
    self.pos[j][0] * self.pos[j + i][1] / l_j
dis2 = abs(delta_yj) * x2 - delta_xj * y2 + self.pos[j + i][0] * self.pos[j][1]
```
...
打印板凳光图像(有板凳)
def print_ing(self, save_pth=None, magn=None, shov=True, circle=False):
    save_pth 保存路径，默认不保存
    magn 是否放大中心，默认不放大
...
def next_pos(self, theta_n, is_head=False): # theta_n为前一把手角度
    ...
    parai = (2 * pi * l / self.d)**2

    def fn(x):
    return theta_n**2 + x**2 - 2 * theta_n * x * cos(x - theta_n) - parai
...
font2 = FontProperties(fname*/System/Library/Fonts/Supplemental/Songti.ttf*)  
ben_vid = 0.3 # 板聚宽度(m)  
ben_len_head = 3.41 # 龙头板长(m)  
ben_len = 2.20 # 龙身及光尾板长度(m)  
...
* 小圆圆心坐标  
OZx = -r * cos(theta) + R2 * sin(theta + alpha)
```
...
# 当前把手位于盘入螺线
    ...
    yid = -self.vol[n] * cos(theta1)
    par1 = (x1 - d / (2 * pi) * theta2 * cos(theta2)) * xid + (y1 - d / (2 * pi) * theta2 * sin(theta2)) * yid
```
...
# 当前把手位于大圆圆弧
    ...
    xid = self.vol[n] * sin(theta1)
    yid = -self.vol[n] * cos(theta1)
    par1 = (x1 - 0ix - R1 * cos(theta2)) * xid + (y1 - 0iy - R1 * sin(theta2)) * yid
    par2 = (x1 - 0ix - R1 * cos(theta2)) * sin(theta2) - (y1 - 0iy - R1 * sin(theta2)) * cos(theta2)
...
# 当前把手位于小圆圆弧
    ...
    par1 = (x1 - 0zx + R2 * cos(theta2)) * xid + (y1 - 0zy + R2 * sin(theta2)) * yid
    par2 = (x1 - 0zx + R2 * cos(theta2)) * sin(theta2) - (y1 - 0zy + R2 *)
```
...
# 绘制两圆圆心
plt.scatter(O1x, 01y, color='black', marker='.', alpha=0.5)
plt.scatter(O2x, 02y, color='black', marker='.', alpha=0.5)
...
# 打印板凳光图像(有板凳)
def print_ing(self, save_pth=None, magn=None, show=True):
    # save_pth 保存路径，默认不保存
    # magn 是否放大中心，默认不放大
    # show 是否展示，默认展示
...
plt.figure(figsize=(8, 8))
    ...
    # 绘制盘入盘出螺线
plt.plot(x1, yy1, color='red', linewidth=2, label='盘入螺线', alpha=0.5)
plt.plot(x2, yy2, color='blue', linewidth=2, label='盘出螺线', alpha=0.5)
plt.plot([], [], color='black', linewidth=2, label='调头螺线', alpha=0.5)
...
plt.gca().add_artist(circle)
    ...
    # 绘制调头路径
arc1 = patches.Arc((01x, 01y), width=R1 * 2, height=R1 * 2, theta=np.rad2deg(theta - alpha - pi / 2), theta2=np.rad2deg(theta + alpha - pi / 2), color='black', linewidth=2, alpha=0.5)
plt.gca().add_patch(arc1)

    arc2 = patches.Arc((02x, 02y), width=R2 * 2, height=R2 * 2, theta=np.rad2deg(theta - alpha + pi / 2), theta2=np.rad2deg(theta + alpha + pi / 2), color='black',
```
...
plt.scatter(x_lst, y_lst, color='black', marker='ality')
...
# 保存板聚龙图像
def save_ingo(self, start_t, end_t, save_pth="photos2", magn=8):
    for time in np.arange(start_t, end_t + 1, 1):
    print(*time = ", time)
    self.set_time(tine)
    self.print_ing(os.path.join(save_pth, f*(tine).png*), magn=magn, show=False)
...
plt.plot([], [], color='black', linewidth=2, label='调头螺径', alpha=0.5)
...
# 绘制两圆圆心
plt.scatter(Oix, Oiy, color='black', marker=',', alpha=0.5)
plt.scatter(O2x, O2y, color='black', marker=',', alpha=0.5)

# 绘制调头路径
arc1 = patches.Arc((Oix, Oiy), width=R1 * 2, height=R1 * 2, theta1=np.rad2deg(theta-alpha-pi/2), theta2=np.rad2deg(theta+alpha-pi/2), color='black', linewidth=2, alpha=0.5)
plt.gca().add_patch(arc1)
...
col_0 = ['虎头']  
for i in range(1, 222):  
    col_0.append(f[第(1)节龙舟')  
col_0.extend(['虎尾', '虎尾（后）'])  
data = {'': col_0}  
data['横坐标x(n)'] = [ben_pos[0] for ben_pos in dragon.pos]  
data['纵坐标y(n)'] = [ben_pos[1] for ben_pos in dragon.pos]  
...
def sin_pitch(start, end, step, fig1=False, fig2=False):
    for d in np.arange(start, end, step):
    dragon = Dragon(d=d)
    time = dragon.arr_time()
    print(f '当前螺距为(d)=')
    pso = PSO(func=dragon.min_dis, n_dis=1, pop=40, max_iter=50, lb=0, ub=time, w=0.8, c1=0.5, c2=0.5)
    pso.run()
    dint = pso.gbest_y
...
if fig1:
    ...
    yy = np.array([dragon.sin_dis(x) for x in xx])
    plt.figure()
    plt.plot(xx, yy)
    plt.plot(pso.gbest_x, pso.gbest_y, '*r')
    plt.ylabel('时间t=s)', fontproperties=font2, size=12)
    plt.ylabel('*点和B点距其他板集中心线的最短距离', fontproperties=font2, size=12)
    plt.xticks(fontproperties=font, size=12)
    plt.yticks(fontproperties=font, size=12)
...
if fig2:
    ...
    plt.ylabel('粒子群迭代次数', fontproperties=font2, size=12)
...
/**
 *  文件名：problem4_i
 *  用途：求解-100-100z各把手的位置
 *
 *  from dragon2 import *
 *  col_0 = ['克头x (m)', '克头y (m)]
 *  for i in range(1, 222):
    col_0.append(f'第i节龙身x (m)')
    col_0.append(f'第i节龙身y (m)')
 *  col_0.extend('克尾x (m)', '克尾y (m)', '克尾（后）x (m)', '克尾（后）y (m)]';
 *  data = {'': col_0}
 *  dragon = Dragon()
...
col_0 = ['龙头 (n/s)]
for i in range(1, 222):
    col_0.append(f'第i)节虎身 (n/s')
col_0.extend(['龙尾 (n/s)', '龙尾 (后) (n/s)'])
data = {'': col_0}
dragon = Dragon()
for time in np.arange(-100, 101, 1):
    if time % 10 == 0:
...
plt.plot(xx, yy, color='00CSD7', linewidth=2)

plt.xlabel('时间t(s)', fontproperties=font2, size=12)
plt.ylabel('把手最大速度(n/s)', fontproperties=font2, size=12)
plt.xticks(fontproperties=font, size=12)
plt.yticks(fontproperties=font, size=12)
...
# 三分搜索算法(求极大值)
def ternary_search(fn, lb, ub, eps=1e-6):
