...
![](images/c92f2087bc6ef85ef299fd5803f759e64e2e0fda9933baea6bfccb2520952eb7.jpg)  
图1 龙头运动模型
...
$$
\sqrt {(x _ {0} - x _ {1}) ^ {2} + (y _ {0} - y _ {1}) ^ {2}} = L\tag{17}
$$
...
![](images/ce216d9d34ec1d67a652c2bdae13eb760dcf8546bf53225e7323da6908d498fe.jpg)  
图2 把手速度模型
...
$$
y ^ {\prime} (\theta_ {i}) = \alpha (\sin \theta_ {i} + \theta_ {i} \cos \theta_ {i})\tag{22}
$$
...
我们将参数 $d = 0.55\mathrm{m}$ ， $\mathbf{v} = 1\mathrm{m / s}$ 代入模型进行求解，以1s的时间间隔将龙身位置和速度数据记录在文件result1.xlsx中。

以300s时刻为例，我们绘制如图（3）所示的板凳龙位置图，此刻把手均位于螺线上，符合题目要求。

300s时刻板凳龙位置图  
![](images/e38c4d8dbb39bb71346f653eded1ff7f5f506d2c698009ed323153749198a29b.jpg)  
图3 300s“板凳龙”位置
...
![](images/bd7c58237171595bbe1bb2a59c07e1ee27b738d6d2f46a529e786ac3eafd15d4.jpg)  
图4 同心圆轨迹碰撞示意图
...
![](images/916133f89341a75f58b5f973d96553e9e79c7cc54dacfde331f8fb644998949f.jpg)  
图5 板凳顶点示意图

由图（5），第i节龙身后把手指指向第i节龙身前把手的方向为：

$$
e _ {i, i + 1} = \frac {\left(x _ {i} - x _ {i + 1} , y _ {i} - y _ {i + 1}\right)}{l}\tag{33}
$$
...
$$
\left\{ \begin{array}{l} P _ {1} ^ {(i)} = (x _ {i}, y _ {i}) - (l + D _ {l}) \cdot \boldsymbol {e} _ {i, i + 1} + D _ {h} \cdot \boldsymbol {n} _ {i, i + 1} \\ P _ {2} ^ {(i)} = (x _ {i}, y _ {i}) + D _ {l} \cdot \boldsymbol {e} _ {i, i + 1} + D _ {h} \cdot \boldsymbol {n} _ {i, i + 1} \\ P _ {3} ^ {(i)} = (x _ {i}, y _ {i}) + D _ {l} \cdot \boldsymbol {e} _ {i, i + 1} - D _ {h} \cdot \boldsymbol {n} _ {i, i + 1} \\ P _ {4} ^ {(i)} = (x _ {i}, y _ {i}) - (l + D _ {l}) \cdot \boldsymbol {e} _ {i, i + 1} - D _ {h} \cdot \boldsymbol {n} _ {i, i + 1} \end{array} \right.\tag{35}
$$
...
![](images/f2046e02ed4782d2549fe64b61da08d4d5100712fbbac8f29e813a48672fdd8a.jpg)  
图6 碰撞判断模型

当第i节龙身的顶点 $P_{1}^{(i)}$ 、 $P_{2}^{(i)}$ 在 $\square P_{1}P_{2}P_{3}P_{4}^{(j)}$ 内部时，我们判定第i节龙身与第j节龙身碰撞。如图（6）将 $\square P_{1}P_{2}P_{3}P_{4}^{(j)}$ 沿对角线划分为两个三角形，分别判断 $P_{1}^{(i)}$ 、 $P_{2}^{(i)}$ 是否在两个三角形内部，以判断 $P_{1}^{(i)}$ 是否在 $\triangle P_{1}P_{3}P_{4}^{(j)}$ 内部为例进行说明。

判断一个点是否位于三角形内部，可以通过叉乘法 $^{[2]}$ 来实现。构造从三角形顶点到点 $P_{1}^{(6)}$ 的向量，并计算三个向量之间的叉乘，如果叉乘结果都为正，或者都为负，则点 $P_{1}^{(6)}$ 位于三角形内部。

$$
\left\{ \begin{array}{l} \left(\overrightarrow {P _ {1} ^ {(j)} P _ {3} ^ {(j)}} \times \overrightarrow {P _ {1} ^ {(j)} P _ {1} ^ {(j)}}\right) \cdot \left(\overrightarrow {P _ {3} ^ {(j)} P _ {4} ^ {(j)}} \times \overrightarrow {P _ {3} ^ {(j)} P _ {1} ^ {(j)}}\right) > 0 \\ \left(\overrightarrow {P _ {3} ^ {(j)} P _ {4} ^ {(j)}} \times \overrightarrow {P _ {3} ^ {(j)} P _ {1} ^ {(j)}}\right) \cdot \left(\overrightarrow {P _ {4} ^ {(j)} P _ {1} ^ {(j)}} \times \overrightarrow {P _ {4} ^ {(j)} P _ {1} ^ {(j)}}\right) > 0 \\ \left(\overrightarrow {P _ {4} ^ {(j)} P _ {1} ^ {(j)}} \times \overrightarrow {P _ {4} ^ {(j)} P _ {1} ^ {(j)}}\right) \cdot \left(\overrightarrow {P _ {1} ^ {(j)} P _ {3} ^ {(j)}} \times \overrightarrow {P _ {1} ^ {(j)} P _ {1} ^ {(j)}}\right) > 0 \end{array} \right.\tag{36}
$$
...
![](images/14ad0e061ff23d75e58eecc4472b39f8b11506e8ec944edc676c3d34bc3f8e05.jpg)  
图7 判断碰撞算法流程图
...
![](images/41814485d403774b5701a7fce749bfbbb308d17fa2c99ec22c086e25ae032d2b.jpg)  
图8 调头空间示意图

记盘入螺线与调头空间边缘交点为 $(x_{in},y_{in})$ ，由题意，盘出螺线与盘入螺线关于螺线中心呈中心对称，则盘出螺线与调头空间边缘交点为 $(-x_{in},-y_{in})$ ，且切线 $l_{1}/l_{2}$ 。由于 $(x_{in},y_{in})$ 处的切线方向为 $e=(x_{in}',y_{in}')$ ，法线方向为 $n=(-y_{in}',x_{in}')$ ，记前一段圆弧的圆心 $O_{1}$ 坐标为：

$$
O _ {1} = \left(x _ {i n}, y _ {i n}\right) + 2 \lambda \cdot \boldsymbol {n}\tag{39}
$$

其中， $\lambda$ 为参数。由于前一段圆弧的半径是后一段圆弧半径的 2 倍，则后一段圆弧

的圆心坐标 $O_{2}$ 为：

$$
O _ {2} = (- x _ {i n}, - y _ {i n}) - \lambda \cdot n\tag{40}
$$
...
$$
\left| O _ {1} O _ {2} \right| = 3 \lambda | n |\tag{41}
$$
...
![](images/2bd8e50a5e78e8151262956921b45dcef091c9a33e9739e1cf4134e143fd8793.jpg)  
图9 调头空间内几何关系示意图
...
定义龙头开始调头的径向距离为调头半径，记为r；又弧长 $C=H\beta/\sin\beta$ ，其中 $\beta=\pi-2\arcsin(H/2r)$ ，可知弧长与调头半径正相关。根据假设，“板凳龙”在行进过程中不会倒退，小圆弧的直径大于等于龙头前后把手之间的距离，则 $2\min\{r_{1},r_{2}\}\geq L$ 。针对不同约束，可对最小弧长进行求解：
...
根据式（11）的解析解，修改初始条件为 $\theta|_{t=0} = \theta_m$ ，其中 $\theta^*$ 表示龙头进入调头空间时的极角。此时， $\theta$ 满足：

$$
\begin{array}{l} \theta \sqrt {\theta^ {2} + 1} + \ln (\theta + \sqrt {\theta^ {2} + 1}) = \frac {2 v}{\alpha} t + C _ {1} \\ C _ {1} = \theta_ {i n} \sqrt {\theta_ {i n} ^ {2} + 1} + \ln (\theta_ {i n} + \sqrt {\theta_ {i n} ^ {2} + 1}) \end{array}\tag{45}
$$

当“板凳龙”进入第一段圆弧，即 $0<t<\frac{\beta}{\omega_{1}}$ 时，由于此时“板凳龙”沿第一段圆弧以角速度 $\omega_{1}=\frac{v}{R_{1}}$ 做顺时针运动，则：

$$
F (t) = \binom {x _ {\mathrm{in}} - 2 \lambda y _ {\mathrm{in}} ^ {\prime} + R _ {1} \cos (\gamma_ {1} - \omega_ {1} t)} {y _ {\mathrm{in}} - 2 \lambda x _ {\mathrm{in}} ^ {\prime} + R _ {1} \sin (\gamma_ {1} - \omega_ {1} t)} ^ {T}\tag{46}
$$
...
与第一种情况一致，修改初始条件为 $\theta \Big|_{t = \beta \left(\frac{1}{\omega_1} +\frac{1}{\omega_2}\right)} = \theta_{im}$ ，其中 $\theta_{im}$ 表示龙头进入调头空间时的极角。此时， $\theta$ 满足：

$$
\begin{array}{c} \theta \sqrt {\theta^ {2} + 1} + \ln (\theta + \sqrt {\theta^ {2} + 1}) = \frac {2 v}{\alpha} t + C _ {2} \\ C _ {1} = \theta_ {i n} \sqrt {\theta_ {i n} ^ {2} + 1} + \ln (\theta_ {i n} + \sqrt {\theta_ {i n} ^ {2} + 1}) - \frac {2 v \beta}{\alpha} \left(\frac {1}{\omega_ {1}} + \frac {1}{\omega_ {2}}\right) \end{array}\tag{49}
$$
...
$$
F ^ {\prime} (t) = \left(x ^ {\prime}, y ^ {\prime}\right) = \left\{ \begin{array}{l} \left(\alpha (\cos \theta (t) - \theta (t) \sin \theta (t))\right) ^ {T}, - 1 0 0 <   t \leqslant 0 \\ \left(\alpha (\sin \theta (t) + \theta (t) \cos \theta (t))\right) \\ \left(y (t) - y _ {m} - 2 \lambda x _ {m} ^ {\prime}, - x (t) + x _ {m} - 2 \lambda y _ {m} ^ {\prime}\right), 0 <   t <   \frac {\beta}{\omega_ {1}} \\ (- y (t) - y _ {m} - 2 x _ {m} ^ {\prime}, x (t) + x _ {m} - 2 y _ {m} ^ {\prime}), \frac {\beta}{\omega_ {1}} <   t <   \beta \left(\frac {1}{\omega_ {1}} + \frac {1}{\omega_ {2}}\right) \\ \left(\alpha (\cos (\theta (t) + \pi) - \theta (t) \sin (\theta (t) + \pi))\right) ^ {T}, \beta \left(\frac {1}{\omega_ {1}} + \frac {1}{\omega_ {2}}\right) <   t <   1 0 0 \\ \left(\alpha (\sin (\theta (t) + \pi) + \theta (t) \cos (\theta (t) + \pi))\right), \beta \left(\frac {1}{\omega_ {1}} + \frac {1}{\omega_ {2}}\right) <   t <   1 0 0 \end{array} \right.\tag{51}
$$
...
最大速度随时间变化情况  
![](images/f37d32b316973aef1c568eb8ff0e6868b7e71b9589393ee8b1bb516aefff3b87.jpg)  
图10 最大速度随时间变化情况
...
![](images/5883939845505d41536d38bf58ee6a7684e83dade28bc99ea9d3721d87a82aed.jpg)  
...
行进速度对最大速度位置的影响  
![](images/6b6d723f5c4666baeb7ef01dd251d72f83b1285bbbdc8a0f17a496fdc1aa5613.jpg)  
图12 行进速度对最大速度位置的影响
...
2. 最大速度条件验证：计算中间值 $mid = (l + r) / 2$ ，并计算 $v_{\mathrm{max}}$ 与 $2\mathrm{m / s}$ 进行比较。
...
![](images/cb24db14f4792790919dfbd23645b13229903ab2fb1c3bd47d9def0f721a0983.jpg)  
图13 求解结果位置误差
...
![](images/c215a41af4caead95689528aea56819e99d30694d3505c5984ffe6ad138bc504.jpg)
...
附录一 支撑文件列表
Problem1\_1.py # 圆半径近似计算位置和速度
Problem1\_2.py # 精确计算位置和速度
Problem1\_3.py # 差分算法计算速度
Problem1\_4.py # 误差对比
Problem2\_1.py # 同心圆估计碰撞时间
Problem2\_2.py # 计算碰撞时刻，保存数据，绘制示意图
Problem3\_1.py # 二分法计算最小螺距
Problem4\_1.py # 精确计算问题四
Problem4\_2.py # 差分算法计算速度
Problem4\_3.py # 计算最短圆弧
Problem5\_1.py # 二分法求最大速度
Plot1\_1.py # 绘制300s时刻的精确版本的板凳龙位置图
Plot4\_1.py # 绘制调整路径示意图
Plot5\_1.py # 绘制最大速度随时间变化分布图
Plot5\_2.py # 绘制最大速度随速度变化分布图
Table.py # 导出论文结果
result1.xlsx # 问题一结果
result2.xlsx # 问题二结果
result4.xlsx # 问题四结果
...
def crash(theta):
    ...
    # 碰撞点
    if i == 0:
    point1 = vertices(theta[i], theta[i+1], 3.41)[I]
```
...
def speed(x, y, theta):
    ...
    d = np.array([cos(theta[i+1]) - theta[i+1]*sin(theta[i+1]), sin(theta[i+1]) + theta[i+1]*cos(theta[i+1]]))    # 这一把手速度方向
    d = d / np.linalg.norm(d)
    vector = np.array([x[i+1] - x[i], y[i+1] - y[i]])    # 板聚方向
    v1 = root(lambda v: np.dot(v1*d1, vector) - np.dot(v*d, vector), v1).x[0]
    speed.append(v1)
    return speed
...
def draw(x, y, theta, t):
    ...
    l = 2.2
    p = plt.Polygon(xy=vertices(theta[i], theta[i+1], 1), color='#C82423', alpha=0.8)
    axes3.add_patch(p)
...
for j in [mess[0], mess[2]]:
    ...
    l = 2.2
    p = plt.Polygon(xy=vertices(handle_theta[j], handle_theta[j+1], 1), color='r', alpha=0.8)
    axes3.add_patch(p)
plt.plot(x1, y1, linestyle='--', color='#2878B5', label='螺旋线')
...
theta, 0
    ...
    x, y = x0 - 2*1*rn[1] + g1 * cos(t1 - t * w1), y0 + 2*1*rn[0] + g1 * sin(t1 - t * w1),
    return x, y, sqrt(x**2 + y**2)/alpha, 1
    elif t <= beta * (1/w1 + 1/w2):
    x, y = -x0 + 1*rn[1] + g2*cos(t2 + (t - beta / w1) * w2), -y0 - 1*rn[0] + g2*sin(t2 + (t - beta / w1) * w2)
...
x, y = x0 - k*l * rn[1] + g1 * cos(t1 - t/g1), y0 + k * l * rn[0] + g1 * sin(t1 - t/g1)
