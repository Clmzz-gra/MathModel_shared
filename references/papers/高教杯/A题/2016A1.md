...
![](images/2b666d57baf19989c40429ef486523d7e40fbcd12355211c0353277a00c4aa2a.jpg)  
图 1 传输节点示意图
...
![](images/a539eed1fc13c32f9e5c1cc6d2a755897b2816a8794e505e0f279fac924679ba.jpg)  
图 2 浮标受力分析示意图

由浮标受力分析示意图（图 2）可知，浮标所受钢管的拉力 $\mathrm { T } _ { 1 }$ 、风力 $\mathsf { F } _ { \mathbb { X } }$ 、重力 $G _ { \sqrt { \sharp } , \sharp , \overline { { \jmath } } }$ 和浮力 $\mathrm { F _ { \mathrm { \sqrt { \frac { \pi } { \hbar } } } \mathrm { / \cdot } } }$ 的合力应为零，即：

$$
\vec {\mathrm{T}} _ {1} + \vec {\mathrm{F}} _ {\mathrm{风}} + \vec {\mathrm{F}} _ {\mathrm{浮标}} + \vec {\mathrm{G}} _ {\mathrm{浮标}} = 0\tag{5-1}
$$
...
![](images/db55e8c90636f63a4d86399f7a9695892e6c8b1bfcbb8d5625a58dfc9494d6d5.jpg)  
图 3 钢管受力分析示意图

由钢管受力分析示意图（图3）可知，钢管所受拉力 $\mathrm { T } _ { 1 }$ 、 $\mathrm { T } _ { 2 }$ 、重力 ${ \sf G } _ { 1 }$ 和浮力$\mathsf { F } _ { 1 } \ast _ { \overrightarrow { f } }$ 的合力应为零，即：

$$
\vec {\mathrm{T}} _ {1} + \vec {\mathrm{T}} _ {2} + \vec {\mathrm{F}} _ {1 \text {浮}} + \vec {\mathrm{G}} _ {1} = 0\tag{5-5}
$$
...
其中 $G _ { 1 }$ 为第一节钢管所受的重力，可通过 ${ \sf G } _ { 1 } = { \sf m } _ { 1 }$ g计算得到， $\mathrm { T } _ { 2 }$ 为第二节钢管对第一节钢管的拉力，与水平方向的倾斜角度记为 $\theta _ { 2 }$ 。

由于钢管是刚性物体，受到多个力的作用，所以需要对第一节钢管的力矩的平衡性进行分析（如图4所示）：

![](images/da62a644a12d09c7303b2a6d6283e354e2483fd94263e1a4c605db081a10684d.jpg)  
图 4 钢管力矩平衡分析示意图

由力矩关系定理和钢管力矩平衡分析示意图（图4）知，可分别以节点 2和节点1作为参考点，对钢管的力矩进行力矩平衡性分析可得：

$$
\left\{ \begin{array}{l} {{\mathrm{L} _ {1} \mathrm{T} _ {1} \sin (\theta_ {1} - \varphi_ {1}) = \frac {1}{2} \mathrm{L} _ {1} (\mathrm{G} _ {1} - \mathrm{F} _ {1 \text {浮}}) \cos \varphi_ {1}}} \\ {{\mathrm{L} _ {1} \mathrm{T} _ {2} \sin (\varphi_ {1} - \theta_ {2}) = \frac {1}{2} \mathrm{L} _ {1} (\mathrm{G} _ {1} - \mathrm{F} _ {1 \text {浮}}) \cos \varphi_ {1}}} \end{array} \right.\tag{5-7}
$$
...
$$
\left\{ \begin{array}{l} \mathrm {L_ {i} T_ {i}} \sin (\theta_ {\mathrm{i}} - \varphi_ {\mathrm{i}}) = \frac {1}{2} \mathrm {L_ {i}} \left(\mathrm {G_ {i}} - \mathrm {F_ {\text {浮} i}}\right) \cos \varphi_ {\mathrm{i}}, (\mathrm{i} = 1, 2, 3, 4) \\ \mathrm {L_ {i} T_ {i + 1}} \sin (\varphi_ {\mathrm{i}} - \theta_ {\mathrm{i+1}}) = \frac {1}{2} \mathrm {L_ {i}} \left(\mathrm {G_ {i}} - \mathrm {F_ {\text {浮} i}}\right) \cos \varphi_ {\mathrm{i}}, (\mathrm{i} = 1, 2, 3, 4) \end{array} \right.\tag{5-9}
$$
...
![](images/fae237afddce1d823b9bdd984f94a8186350bdd78f6bc2e3c21b5145bb2d51c9.jpg)  
图 5 钢桶受力分析示意图

钢桶所受合力应为零，即：

$$
\vec {\mathrm{T}} _ {5} + \vec {\mathrm{T}} _ {6} + \vec {\mathrm{G}} _ {5} + \vec {\mathrm{F}} _ {\mathrm{浮} 5} + \vec {\mathrm{G}} _ {\mathrm{球}} + \vec {\mathrm{F}} _ {\mathrm{浮球}} = 0\tag{5-11}
$$
...
其中 $\mathrm { G } _ { \ddagger \mathparagraph }$ 表示重物球受到的自身重力， ${ \sf G } _ { 5 }$ 表示钢桶受到的自身重力， $\operatorname { F } _ { \ P , \ P , \Phi }$ 表示重物球受到的浮力，可通过公式 $\mathrm { F } _ { \mathrm { i } \ddag \ddag \sqrt { \ddag } } = \rho _ { \mathrm { i } \ddag \uparrow \sqrt { \vphantom { \ddag } } } g \frac { m _ { \ddag \ast } } { \rho _ { \rlap / _ { \ddag \sqrt { \vphantom { \ddag } } } } }$ 求得， $m _ { \mathbb { H } }$ 为重力球的质量，$\rho _ { \mathrm { f s | X | } }$ 为钢材的密度。 $\mathsf { F } _ { \sqrt { \sharp } \ 5 }$ 表示钢桶受到的浮力。

由于钢桶也是刚性物体，受到多个力的作用，所以需要对钢桶的力矩的平衡性进行分析（如图 6所示）：

![](images/629c8e9625e3d1a9c1b5a8a1aae33ac8ac5668e2a74ddce98d2bec695fefd784.jpg)  
图 6 钢桶力矩平衡分析示意图
...
![](images/d32bfa81d9927df4c3026887572f5941430b122a8fd8d17527933ca91f5c0848.jpg)  
图 7 锚链受力分析示意图

由锚链受力分析示意图（图 7）所示，第一节锚链所受合力应该为零，即：

$$
\vec {\mathrm{T}} _ {6} + \vec {\mathrm{T}} _ {7} + \vec {\mathrm{F}} _ {\mathrm{浮} 6} + \vec {\mathrm{G}} _ {6} = 0\tag{5-14}
$$

对钢桶所受的力按水平、竖直两个方向进行分解，可得：

$$
\left\{ \begin{array}{c} {\mathrm{T} _ {7} \cos \theta_ {7} = \mathrm{T} _ {6} \cos \theta_ {6}} \\ {\mathrm{T} _ {7} \sin \theta_ {7} = \mathrm{T} _ {6} \sin \theta_ {6} - (\mathrm{G} _ {6} - F _ {\text {浮} 6})} \end{array} \right.\tag{5-15}
$$

其中 $G _ { 6 }$ 为第一节电焊锚链所受的重力， $\mathrm { T } _ { 7 }$ 为第二节锚链对第一节锚链的拉力，与水平方向的倾斜角度记为 $\theta _ { 7 }$ 。第一节电焊锚链所受的浮力 $F _ { \sqrt { \breve { \mp } } \ 6 }$ 可通过公式$\boldsymbol { F } _ { \uplus \uplus 6 } = \rho _ { \uplus \uplus \uplus 6 } \boldsymbol { g } \frac { m _ { \# \uplus \vartriangle 6 } } { \rho _ { \sharp \vartriangle 6 } }$ 算得， $m _ { \sharp \sharp \sharp }$ 表示单节电焊锚链的质量。

由于电焊锚链也是刚性物体，所以电焊锚链所受拉力的方向与电焊锚链所在方向之间存在夹角，故我们可对第一节电焊锚链进行力矩平衡分析（如图7示）：

![](images/9d30b737abe12619f953ed0dc29fe46d86bf247367e7a273d07995a58221d1f8.jpg)  
图 8 锚链受力分析示意图

由力矩关系定理和钢管力矩平衡分析示意图（图4）知，可分别以节点 7和节点6作为参考点，对钢管的力矩进行力矩平衡性分析可得：

$$
\left\{ \begin{array}{l l} {{\mathrm{L} _ {6} \mathrm{T} _ {6} \sin (\theta_ {6} - \varphi_ {6}) = \frac {1}{2} \mathrm{L} _ {6} (\mathrm{G} _ {6} - F _ {\mathrm{浮} _ {6}}) \cos \varphi_ {6}}} \\ {{\mathrm{L} _ {6} \mathrm{T} _ {7} \sin (\varphi_ {6} - \theta_ {7}) = \frac {1}{2} \mathrm{L} _ {6} (\mathrm{G} _ {6} - F _ {\mathrm{浮} _ {6}}) \cos \varphi_ {6}}} \end{array} \right.\tag{5-16}
$$
...
$$
\left\{ \begin{array}{c} {\mathrm {T_ {i + 1}} \cos \theta_ {\mathrm{i+1}} = \mathrm {T_ {i}} \cos \theta_ {\mathrm{i}}, (\mathrm{i=6,7,} \dots , \mathrm{n})} \\ {\mathrm {T_ {i + 1}} \sin \theta_ {\mathrm{i+1}} = \mathrm {T_ {i}} \sin \theta_ {\mathrm{i}} - (\mathrm {G_ {i}} - \mathrm {F_ {\text {浮} i}}), (\mathrm{i=6,7,} \dots , \mathrm{n})} \end{array} \right.\tag{5-17}
$$
...
$$
\left\{ \begin{array}{l} \mathrm {L_ {i} T_ {i}} \sin (\theta_ {\mathrm{i}} - \varphi_ {\mathrm{i}}) = \frac {1}{2} \mathrm {L_ {i}} (\mathrm {G_ {i}} - \mathrm {F_ {\text {浮} i}}) \cos \varphi_ {\mathrm{i}}, \quad (\mathrm{i} = 6, 7, \dots , \mathrm{n}) \\ \mathrm {L_ {i} T_ {i + 1}} \sin (\varphi_ {\mathrm{i}} - \theta_ {\mathrm{i+1}}) = \frac {1}{2} \mathrm {L_ {i}} (\mathrm {G_ {i}} - \mathrm {F_ {\text {浮} i}}) \cos \varphi_ {\mathrm{i}}, \quad (\mathrm{i} = 6, 7, \dots , \mathrm{n}) \end{array} \right.\tag{5-18}
$$

将公式（5-18）进行化简，便可得到钢管与海平面夹角 $\varphi _ { \mathrm { i } } \mathrm { ( i = 6 , 7 , ~ \cdots , n ) }$ 的递推公式：

$$
\tan \varphi_ {\mathrm{i}} = \frac {2 \mathrm{T} _ {\mathrm{i}} \sin \theta_ {\mathrm{i}} - (\mathrm{G} _ {\mathrm{i}} - \mathrm{F} _ {\text {浮} _ {\mathrm{i}}})}{2 \mathrm{T} _ {\mathrm{i}} \cos \theta_ {\mathrm{i}}}    ,    (\mathrm{i} = 1, 2, 3, 4)\tag{5-10}
$$
...
$$
\mathrm{R} = \sum_ {i = 1} ^ {n} L _ {i} c o s \varphi_ {\mathrm{i}}\tag{5-12}
$$

将上述的力学分析和和力矩平衡的递推公式进行整理，可得：

$$
\left\{ \begin{array}{l l} \left\{T _ {1} \cos \theta_ {1} = 0. 6 2 5 \times D (2 - h) v ^ {2} \right. \\ \left\{T _ {1} \sin \theta_ {1} = \rho_ {\text {海水}} g \pi r ^ {2} h - m _ {\text {浮标}} g \right. \\ \left\{T _ {i + 1} \cos \theta_ {i + 1} = T _ {i} \cos \theta_ {i} \right. \\ \left\{T _ {i + 1} \sin \theta_ {i + 1} = T _ {i} \sin \theta_ {i} - (G _ {i} - F _ {i \text {浮}}) \right. & (i \neq 5) \\ \left\{T _ {6} \cos \theta_ {6} = T _ {5} \cos \theta_ {5} \right. \\ \left\{T _ {6} \sin \theta_ {6} = T _ {5} \sin \theta_ {5} - (G _ {\text {球}} - F _ {\text {浮球}}) - (G _ {5} - F _ {\text {浮} 5}) \right. & (i = 5) \\ \varphi_ {i} = \arctan \left(\frac {2 T _ {i} \sin \theta_ {i} - (G _ {i} - F _ {\text {浮} i})}{2 T _ {i} \cos \theta_ {i}}\right), (i = 1, 2, \dots , n) \\ H = \sum_ {i = 1} ^ {n} L _ {i} s i n \varphi_ {i} + h \\ R = \sum_ {i = 1} ^ {n} L _ {i} c o s \varphi_ {i} \end{array} \right.\tag{5-13}
$$
...
![](images/3d6973c00a2a6155c4edb8ea1c94bb939e85e702b98295c75f028097e7a5ee4d.jpg)  
图 9 吃水深度与海水深度的变化曲线
...
风速12m/s时锚链形状  
![](images/0a13a4e39e12f85923130b7ceaf2b91b971e310307daff84cfe33000b28873fc.jpg)  
图 10 风速12m/s 时锚链的形状

风速24m/s时锚链形状  
![](images/b61c0719985df4dec63593edc7eff054d3921c24c64255710646fdb9191aa92f.jpg)  
图 11 风速24m/s 时锚链的形状
...
$$
\left\{ \begin{array}{l l} \left\{T _ {1} \cos \theta_ {1} = 0. 6 2 5 \times D (2 - h) v ^ {2} \right. \\ \left. T _ {1} \sin \theta_ {1} = \rho_ {\text {海水}} g \pi r ^ {2} h - m _ {\text {浮标}} g \right. \\ \left\{T _ {i + 1} \cos \theta_ {i + 1} = T _ {i} \cos \theta_ {i} \right. \\ \left. T _ {i + 1} \sin \theta_ {i + 1} = T _ {i} \sin \theta_ {i} - (G _ {i} - F _ {i \text {浮}}) \right. & (i \neq 5) \\ \left\{T _ {6} \cos \theta_ {6} = T _ {5} \cos \theta_ {5} \right. \\ \left. T _ {6} \sin \theta_ {6} = T _ {5} \sin \theta_ {5} - (G _ {\text {球}} - F _ {\text {浮球}}) - (G _ {5} - F _ {\text {浮} 5}) \right. & (i = 5) \\ \tan \varphi_ {i} = \frac {2 T _ {i} \sin \theta_ {i} - (G _ {i} - F _ {\text {浮} i})}{2 T _ {i} \cos \theta_ {i}} (i = 1, 2, \dots , n) \\ H = \sum_ {i = 1} ^ {n} L _ {i} s i n \varphi_ {i} + h \\ R = \sum_ {i = 1} ^ {n} L _ {i} c o s \varphi_ {i} \end{array} \right.
$$

$3 6 \mathrm { { m } / \mathrm { { s } } }$ 时，钢桶和各节钢管的倾斜角度，链条形状以及浮标的游动区域。
...
$$
\begin{array}{r l} & {\text {Min} \quad m _ {\text {球}}} \\ & {\text {st.} \left\{ \begin{array}{l l} {\left\{\mathrm{T} _ {1} \cos \theta_ {1} = 0. 6 2 5 \times \mathrm{D} (2 - \mathrm{h}) \mathrm{v} ^ {2} \right.} \\ {\left\{\mathrm{T} _ {1} \sin \theta_ {1} = \rho_ {\text {海水}} g \pi r ^ {2} h - \mathrm{m} _ {\text {浮标}} \mathrm{g} \right.} \\ {\left\{\mathrm{T} _ {\mathrm{i+1}} \cos \theta_ {\mathrm{i+1}} = \mathrm{T} _ {\mathrm{i}} \cos \theta_ {\mathrm{i}} \right.} \\ {\left\{\mathrm{T} _ {\mathrm{i+1}} \sin \theta_ {\mathrm{i+1}} = \mathrm{T} _ {\mathrm{i}} \sin \theta_ {\mathrm{i}} - (\mathrm{G} _ {\mathrm{i}} - \mathrm{F} _ {\mathrm{i浮}}) \right.} & {(i \neq 5)} \\ {\left\{\mathrm{T} _ {6} \cos \theta_ {6} = \mathrm{T} _ {5} \cos \theta_ {5} \right.} \\ {\left\{\mathrm{T} _ {6} \sin \theta_ {6} = \mathrm{T} _ {5} \sin \theta_ {5} - (\mathrm{G} _ {\text {球}} - \mathrm{F} _ {\text {浮球}}) - (\mathrm{G} _ {5} - \mathrm{F} _ {\text {浮5}}) \right.} & {(i = 5)} \\ {\tan \varphi_ {\mathrm{i}} = \frac {2 \mathrm{T} _ {\mathrm{i}} \sin \theta_ {\mathrm{i}} - (\mathrm{G} _ {\mathrm{i}} - \mathrm{F} _ {\text {浮i}})}{2 \mathrm{T} _ {\mathrm{i}} \cos \theta_ {\mathrm{i}}} (\mathrm{i=1,2,···,n})} \\ {\mathrm{H} = \sum_ {i = 1} ^ {n} L _ {i} s i n   \varphi_ {\mathrm{i}} + h} \\ {\mathrm{R} = \sum_ {i = 1} ^ {n} L _ {i} c o s   \varphi_ {\mathrm{i}}} \\ {\varphi_ {2 1 5} \leq 1 6 ^ {\circ}} \\ {9 0 ^ {\circ} - \varphi_ {5} \leq 5 ^ {\circ}} \end{array} \right.} \end{array}
$$

## (3) 求满足设计要求的最小重物球质量

对此，以重物球的质量为搜索变量，钢桶的倾斜角度小于 $5 ^ { \circ }$ °和最后一节锚链的锚链与海床的夹角小于 16°为约束条件，采用二分法对海水深度 18m 进行逼近，得到的重物球的质量即所需的最小质量，其具体步骤如下：
...
d、利用二分法对海水深度 18m进行逼近，求满足钢桶的倾斜角度 $\Psi _ { 5 } < 5 ^ { \circ }$ 最后一节锚链与海平面的夹角 $\varphi _ { 2 1 5 } < 1 6 ^ { \circ }$ 和海水深度Hc满足 $| \mathrm { H c } - 1 8 | < \mathrm { e p s }$ 件的浮标吃水深度和重物球的质量，具体流程见二分法的步骤见图（图12）；

e、记录满足约束条件的浮标吃水深度和重物球的质量。

![](images/5ec91fcbe595e604c30b2d3df61fc6bef034f04d302e7b1c4d773c1ed4848a71.jpg)  
图 12 二分法流程示意图
...
$$
\left\{ \begin{array}{l} \left\{ \begin{array}{c} \mathrm{T} _ {1} \cos \theta_ {1} = 0. 6 2 5 \mathrm{D} (2 - \mathrm{h}) \mathrm{v} ^ {2} + 3 7 4 \mathrm{Dhv} ^ {2} \\ \mathrm{T} _ {1} \sin \theta_ {1} = \rho_ {\text {海水}} g \pi r ^ {2} h - \mathrm{m} _ {\text {浮标}} \mathrm{g} \end{array} \right. \\ \left\{ \begin{array}{l} \mathrm{T} _ {\mathrm{i} + 1} \cos \theta_ {\mathrm{i} + 1} = \mathrm{T} _ {\mathrm{i}} \cos \theta_ {\mathrm{i}} + \overrightarrow {\mathrm{F}} _ {\text {水} \mathrm{i}} \\ \mathrm{T} _ {\mathrm{i} + 1} \sin \theta_ {\mathrm{i} + 1} = \mathrm{T} _ {\mathrm{i}} \sin \theta_ {\mathrm{i}} - \left(\mathrm{G} _ {\mathrm{i}} - \mathrm{F} _ {\text {浮} \mathrm{i}}\right) \\ (\mathrm{i} \neq 5) \end{array} \right. \\ \left\{ \begin{array}{l} \mathrm{T} _ {6} \cos \theta_ {6} = \mathrm{T} _ {5} \cos \theta_ {5} + \mathrm{F} _ {\text {水} 5} + \mathrm{F} _ {\text {水球}} \\ \mathrm{T} _ {6} \sin \theta_ {6} = \mathrm{T} _ {5} \sin \theta_ {5} - \left(\mathrm{G} _ {\text {球}} - \mathrm{F} _ {\text {浮球}}\right) - \left(\mathrm{G} _ {5} - \mathrm{F} _ {\text {浮} 5}\right) \\ (\mathrm{i} = 5) \\ \varphi_ {\mathrm{i}} = \arctan \left(\frac {2 \mathrm{T} _ {\mathrm{i}} \sin \theta_ {\mathrm{i}} - (\mathrm{G} _ {\mathrm{i}} - \mathrm{F} _ {\text {浮} \mathrm{i}})}{2 \mathrm{T} _ {\mathrm{i}} \cos \theta_ {\mathrm{i}}}\right) \\ (\mathrm{i} = 1, 2, \dots , n) \\ H = \sum_ {i = 1} ^ {n} L _ {i} s i n \varphi_ {\mathrm{i}} + h \\ R = \sum_ {i = 1} ^ {n} L _ {i} c o s \varphi_ {\mathrm{i}} \\ 1 6 \leq H \leq 2 0 \\ \varphi_ {\mathrm{n}} \leq 1 6 ^ {\circ} \\ 9 0 - \varphi_ {5} \leq 5 ^ {\circ} \end{array} \right.
$$
...
若要分析重物球质量对评价指标的影响，可以给定等跨度为 1000 的 4 个重物球质量 1280(kg)、2280(kg)、3280(kg)、4280(kg)，在模型二的条件下，假设其他变量分别为固定值，即II号锚链、锚链总长 $\mathrm { L } = 2 2 . 0 5 \mathrm { m }$ 、海水深度H = 18m、水流速度为1.5m/s、风速为36m/s；可以得到不同重物球质量对评价指标的影响，具体数据见下表：
...
从前面问题的分析可得，已选定锚链型号、风速、水流速度，因此这里的海水深度只与锚链节数和重物球的质量有关。因此按照增加链条节数和重物球质量的方法，满足判断条件：○1钢桶与海平面竖直方向夹角小于 5度；○2最后一条锚链与海平面水平方向夹角小于 16 度。并且题目中要求使得浮标的吃水深度和游动区域及钢桶的倾斜角度尽可能小。以水深为H = 16m为例，具体步骤如下：
...
最终记录的链条节数和重物球质量，作为在水深为H = 16m时既满足约束条件○1、○2，同时又满足题目要求使得浮标的吃水深度和游动区域及钢桶的倾斜角度尽可能小。同理可以将该步骤应用于计算水深为H = 20m中，利用 MATLAB 编程计算得到满足要求链条节数和重物球质量。具体数据见下表：
...
![](images/f32a7495fe84f2e450996519cb54a320d944545061e93c13a351597a50cb2c46.jpg)  
图 13 水深 16米锚链形状

风速36m/s、海水流速1.5m/s锚链形状  
![](images/962210c55ec3bd2bee26ca701028e205286373939564eb07a9ee504ff0dc8347.jpg)  
图 14 水深 20 米锚链形状

风速36m/s、海水流速1.5.m/s时备链形状图  
![](images/fe74e497a1f1d0b5b3f91914a39120a4b4b47d63dc060dbdb6440a74891c3434.jpg)  
图 15 锚链形状
...
$$
\mathrm{L} _ {\mathrm{i}} \mathrm{T} _ {\mathrm{i+1}} \sin (\varphi_ {\mathrm{i}} - \theta_ {\mathrm{i+1}}) = \frac {1}{2} \mathrm{L} _ {\mathrm{i}} (\mathrm{G} _ {\mathrm{i}} - \mathrm{F} _ {\mathrm{浮} _ {\mathrm{i}}}) \cos \varphi_ {\mathrm{i}} (\mathrm{i} = 1, 2, 3, 4)\tag{5-20}
$$
...
$$
\mathrm{L} _ {5} \mathrm{T} _ {6} \sin (\varphi_ {5} - \theta_ {6}) + \mathrm{L} _ {5} (\mathrm{G} _ {\mathrm{球}} - \mathrm{F} _ {\mathrm{浮球}}) \sin \left(\frac {\pi}{2} - \varphi_ {5}\right) = \frac {1}{2} \mathrm{L} _ {5} (\mathrm{G} _ {\mathrm{球}} - \mathrm{F} _ {\mathrm{浮球}}) \cos \varphi_ {5}\tag{5-}
$$
...
$$
\tan \varphi_ {\mathrm{i}} = \frac {2 \mathrm{T} _ {\mathrm{i+1}} \sin \theta_ {\mathrm{i+1}} + (\mathrm{G} _ {\mathrm{i}} - \mathrm{F} _ {\text {浮} \mathrm{i}})}{2 \mathrm{T} _ {\mathrm{i+1}} \cos \theta_ {\mathrm{i+1}}} \big (\mathrm{i} = 6, 7, \dots , \mathrm{n} \big)\tag{5-21}
$$
...
对应海深镭链与海床夹脚变化曲线  
![](images/471951cb9a2bbd20ca9f420a8df1073311891e4e50e7af0421c31b7ce59d5765.jpg)
...
![](images/bc2830ef526980ade4f3a3d5a0e48133143535c2252674664bdf5089cdbce87a.jpg)
...
v2=0.15^2*pi*1;
zhunzhongli=-1025*g*v2+m(i)*g;
zhunzhongli=[zhunzhongli1,zhunzhongli];
elseif i>=7
    z1=z11(q).*cd(q);
    v3=z1./7850;
    zhunzhongli=-1025*g*v3+m(i)*g;
...
for h=h3
    ...
    for j=2:lts+5
    if j<=5
    fy(j)=fy(j-1)-zhunzhongli1(j); %准重力的编号从浮
```
