...
def render_convergence_stats(history: list):
    ...
    st.subheader("收敛统计")
    best_energies = np.array([-h.best_fitness for h in history])
    ...
    for t in thresholds:
        ...
        if len(idx_arr) > 0:
            ...
            results.append({"相对误差": f"{t*100:.1f}%", "到达迭代": "未到达", "总迭代占比": "-"})

    st.dataframe(pd.DataFrame(results), use_container_width=True, hide_index=True)


def render_swarm_stats(history: list):
    """最终粒子群统计。"""
    if not history:
        return
    last = history[-1]
    ...
    st.caption(f"当前惯性权重: {last.inertia:.4f} | 平均速度: {last.avg_velocity:.4g}")
...
def render_export_section(result, history: list, func_name: str):
    ...
    with col1:
        ...
        writer.writerow(["iteration", "best_energy", "mean_energy", "worst_energy",
                          "best_fitness", "diversity", "avg_velocity", "inertia", "elapsed"])
        for h in history:
            writer.writerow([h.iteration, -h.best_fitness, -h.mean_fitness,
                             -h.worst_fitness, h.best_fitness, h.diversity,
                             h.avg_velocity, h.inertia, h.elapsed])
        st.download_button("下载 CSV", csv_buf.getvalue(),
                           file_name=f"pso-{func_name}-history.csv",
    ...
    with col2:
        ...
        st.download_button("下载 JSON", json.dumps(json_data, indent=2, ensure_ascii=False),
                           file_name=f"pso-{func_name}-result.json",
