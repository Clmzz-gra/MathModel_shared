...
2. **把账号发给队长**：队长把你加进仓库的协作者名单（队长操作，你只需要把 GitHub 用户名发给他）
3. **克隆仓库**（在你电脑上任意位置执行）：
   ```
...
| `.trae/skills/math/` | 数学讲解 skill（知识脉络讲解方法） |

### 资源区（不属于共享部分）

**其余任何目录都是"资源区"**——想分享什么就新建目录放进去（如 `resources/`、`素材/`、`草稿/`），随便命名。资源区的内容**只在共享仓内流通，不会同步到队长的核心仓**。

| 区域 | 规则 |
|------|------|
| 共享部分（上表目录） | 正式资产，会双向同步；改动需谨慎 |
| 资源区（你新建的目录） | 随意存放、随意分享；不影响共享部分 |

**规则**：临时资源、参考材料放**资源区**（新建目录）；**不要把临时文件塞进 `references/` 等共享目录**——那会被同步走。
...
## 五、受保护文件（只能通过 Pull Request 修改）

以下文件**不能直接 push**，必须走 **Pull Request（PR）** 流程（由队长审查后合并）：

- `knowledge/expression-library.md`（表达库——人类维护的写作资产）

> （如有其他受保护文件，队长会在此补充。直接 push 会被 GitHub 拒绝，这是正常现象。）
