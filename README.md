...
为了让别人知道你分享了什么，**每次在资源区分享，必须同时**：

1. **建目录**：一个资源一个目录（如 `resources/2024-B优秀论文/`）
2. **放说明文件**：目录内放一个 `说明.md`，写明 4 件事：
   ```
   # 资源名称
   ...
   - 上传：你的名字 + 日期
   ```
3. **登记索引**：在仓库根目录的 [资源索引.md](资源索引.md) 里加一行（路径 + 一句话说明）
4. **写清 commit 信息**：如 `资源: 添加 2024-B 优秀论文合集`

> 说明文件可以很简单，几句话就够。没有说明的资源别人不知道怎么用，等于白传。
