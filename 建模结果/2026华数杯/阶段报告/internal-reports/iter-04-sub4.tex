...
全电价贪心——窗口内选最低价小时开工，无储能/卖电）。B1/B2 为纯启发式下界，惩罚不计入成本
（\texttt{s4-baseline-heuristic.pkl} \texttt{b1}/\texttt{b2} 字段）；B2 因 GPU 容量冲突回退最早小时
5{,}039 个任务（\texttt{b2.n\_late}，回退仍满足 latest 截止、QoS 100\%，§9 未闭合项 6）。
...
\textbf{注}：B1/B2 为无储能纯贪心基线，惩罚不计入表中成本（\texttt{s4-baseline-heuristic.pkl}）；主解
超容仅 1h（318.7 GH），B1 无容量回退（\texttt{b1.n\_late}=0），B2 容量回退 5{,}039 个任务
（\texttt{b2.n\_late}，回退仍满足 latest 截止、QoS 100\%，§9 未闭合项 6）。成本比较为近似口径
（基线超容惩罚不入成本，§5.4）。}
\label{tab:baseline}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{利用率锚定}：受限消纳口径下新能源利用率不可作为优化目标，论文需说明「利用率=评价
        指标而非目标」的边界。
  \item \textbf{超容惩罚不入成本}：\texttt{cost\_main} 不含 $10^6$ 元/GH 惩罚项；B1 无容量回退
        （\texttt{b1.n\_late}=0）、B2 容量回退 5{,}039 个任务（\texttt{b2.n\_late}，\texttt{
        s4-baseline-heuristic.pkl} 字段）——回退任务按最早小时开工仍满足 latest 截止（QoS 100\%），
        但可能 GPU 超容且其惩罚不入成本，故「优于 B2」被系统性高估（近似口径）。
\end{enumerate}
...
\texttt{cost\_final}，$\theta{=}0.3$ 结构、50 轮/区；A/B/C/D 无释放任务，A/B 为 $\theta{=}0.3$
结构值（禁改派、承接任务少于主解，故低于主解 578.28/530.06），C/D 与主解相同 512.91/142.37）。
终值 2139.60M；E/F 终值超容 slack 30{,}953/30{,}944 GH（\texttt{curve} 末轮
\texttt{slack\_gh} 字段）。}
\label{tab:lns}
\end{table}
...
\begin{itemize}
  \item $\theta{=}0.3$ 结构总成本 \textbf{2139.60M}，比主解 2166.64M 低 27M（1.2\%）——但该比较含
        ...
        仍含超容（slack 各 $\approx 31$k GH），主解超容仅 318.7 GH（§3.1 口径声明）；（v）A/B
        表注原误标「沿用主解值」，实为 $\theta{=}0.3$ 结构值（已纠正，见 tab:lns 表注）。按主解
        A/B 值替换后合计 2165.58M：27M 差异中约 26M 来自 A/B 结构口径（禁改派承接任务少），仅约
        1M 来自 E/F 选时改进。故 §4.2「EDF 预填未隐没更优解」结论收窄为\textbf{对照集内
        （B1/B2）成立}。
  ...
  \item 哨兵项：B1/B2 基线容量回退计数 \texttt{b1.n\_late}=0 / \texttt{b2.n\_late}=5{,}039
        （\texttt{s4-baseline-heuristic.pkl}，不计入 \texttt{cost\_main\_M}）；每轮子 MILP gap$\approx 0$；
        LNS 终值 E/F 区仍含超容（slack 各 $\approx 31$k GH），2139.60M
...
\begin{enumerate}
  ...
  \item \textbf{场景 eps-0.95 仅 D 可行}：报告需明确「碳约束压降仅 D 区可行」是区域碳排刚性
        ...
        不可行。
  \item \textbf{B2 基线 QoS 口径已澄清}（建模复核 2026-08-14）：\texttt{s4-baseline-heuristic.pkl}
        \texttt{b2} 中 \texttt{qos\_pct}=100.0 与 \texttt{n\_late}=5{,}039 不矛盾——\texttt{n\_late}
        是「GPU 容量冲突时回退到最早小时（lo=arrive）」的\textbf{回退计数}，非迟到任务数；回退后仍满足
        \texttt{h0+dur$\le$latest} 截止，故 QoS 100\% 自洽。论文按此口径表述，不再视为未决。
\end{enumerate}
\endgroup
