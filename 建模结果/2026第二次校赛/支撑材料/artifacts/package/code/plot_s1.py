...
ax.set_ylabel("AUC")
ax.set_ylim(0, 1.0)
ax.set_title("三数据集 L2 / RF / 单特征基线 AUC 对比（5 折 CV，误差棒表示折间标准差）")
ax.axhline(0.5, color=C_BLACK, lw=0.8, ls=":")
ax.text(2.42, 0.515, "Dummy 多数类基线 0.5", color=C_BLACK, fontsize=7, va="bottom", ha="right",
...
for i, (_, _, key) in enumerate(calibers):
    if key == selected:
        ...
        ax.text(i, 0.98, "主口径", ha="center", va="top", fontsize=8, color=C_BLACK)
ax.set_xticks(x)
ax.set_xticklabels([f"{l}\n(n={n})" for l, n in zip([c for _, c, _ in calibers], n_ad)])
ax.set_ylabel("AUC")
ax.set_ylim(0, 1.0)
ax.set_title("Zeller small_adenoma 四口径敏感性对比（L2 与 RF，阴影为主口径）")
ax.axhline(0.5, color=C_BLACK, lw=0.8, ls=":")
ax.legend(frameon=False, loc="upper left", bbox_to_anchor=(0.02, 0.98), ncol=2)
...
for ax, d, s in zip(axes, DS, DS_SHORT):
    ...
    names = [species_name(feature_names[i]) for i in idx]
    vals = coef[idx]
...
for ax, d, s in zip(axes, DS, DS_SHORT):
    ...
    ax.text(t_best + 0.01, 0.03, f"Youden J {j_best:.3f}", color=C_GRAY,
...
fig.suptitle("L2 主模型阈值-指标曲线（5 折 CV OOF，虚线为 Youden 最优阈值）", fontsize=12)
