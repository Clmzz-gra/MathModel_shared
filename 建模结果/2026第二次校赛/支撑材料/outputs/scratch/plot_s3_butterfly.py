...
y = 0
ax.barh(y, -n_cons, height=0.55, color=C_BLUE, label="方向一致", zorder=3)
ax.barh(y, n_flip, height=0.55, color=C_ORANGE, label="方向翻转", zorder=3)

# 坐标轴清理：隐藏 y 轴、上下/右脊线；数轴保持底部（不穿过条形）
...
# 数轴刻度：0 为原点，每 300 一个刻度；蓝/黄数值并入刻度体系（xticklabels 着色）
# 百分比省略（附录 caption 已含 51.2%/48.8%），避免 387/300、369/300 间距过小导致标签重叠
tick_step = 300
ticks = list(range(-int(xmax // tick_step) * tick_step,
                   int(xmax // tick_step) * tick_step + 1, tick_step))
all_ticks = sorted(set(ticks + [-n_cons, n_flip]))
ax.set_xticks(all_ticks)
labels = []
for v in all_ticks:
    if v == -n_cons:
        labels.append(str(n_cons))
    elif v == n_flip:
        labels.append(str(n_flip))
    else:
        labels.append(str(v))
tl = ax.set_xticklabels(labels, fontsize=9)
# 蓝/黄着色：方向一致刻度蓝色、方向翻转刻度橙色
for v, t in zip(all_ticks, tl):
    if v == -n_cons:
        t.set_color(C_BLUE)
    elif v == n_flip:
        t.set_color(C_ORANGE)
ax.set_xlabel("标志物数量", fontsize=10)

handles, labels = ax.get_legend_handles_labels()
fig.subplots_adjust(top=0.82, bottom=0.18)  # 顶部为标题+图例留出空间
fig.suptitle("共享标志物跨疾病方向一致性（387 vs 369，接近随机）", y=0.97, fontsize=11)
