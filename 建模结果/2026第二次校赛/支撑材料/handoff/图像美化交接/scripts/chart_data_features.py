...
ax.bar(edges[:-1], counts, width=0.05, color=colors, edgecolor="white", linewidth=0.5,
       align="edge", label="特征数")
ax.axvline(0.95, color="#111111", linestyle="--", linewidth=1.2)
ax.text(0.95, max(counts) * 0.95, "0.95 阈值", color="#111111", fontsize=10,
        ha="right", va="top", rotation=90)
ax.annotate(f"{n_gt95} 个特征零值占比 >95%（剔除）", xy=(0.975, 1060), xytext=(0.58, 950),
            fontsize=10, color=C_DISEASE,
            arrowprops=dict(arrowstyle="->", color=C_DISEASE, lw=1.2))
ax.annotate(f"保留 {n_keep} 特征", xy=(0.45, 0), xytext=(0.42, 300),
            fontsize=10, color=C_HEALTH,
            arrowprops=dict(arrowstyle="->", color=C_HEALTH, lw=1.2))
...
ax.text(med_log, max(counts) * 0.95, f"中位数 {np.median(nz):.4f} (log10≈{med_log:.2f})",
        color=C_DISEASE, fontsize=10, ha="right", va="top")
# 范围标注
ax.text(0.98, 0.98, f"范围 {nz.min():.1e} ~ {nz.max():.2f}\n（跨约 {int(np.ceil(log_nz.max()-log_nz.min()))} 个数量级）",
        transform=ax.transAxes, ha="right", va="top", fontsize=9, color=C_GREY,
