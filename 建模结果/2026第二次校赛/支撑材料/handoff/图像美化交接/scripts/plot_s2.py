...
for ax, d in [(ax_crc, "CRC"), (ax_ibd, "IBD"), (ax_ob, "Obesity")]:
    ...
    names = [species_name(f["feature"]) for f in sf]
...
for d in DISEASES:
    ax.plot(tau_grid, tau_counts[d], marker="o", lw=1.8, color=DISEASE_COLOR[d],
            label=f"{DISEASE_CN[d]}", zorder=3)
    for i, (x, y) in enumerate(zip(tau_grid, tau_counts[d])):
        idx_d = DISEASES.index(d)
        dx = (-16, 16, 0)[idx_d] if d != "Obesity" else 0
        dy = (8, 18, 9)[idx_d]
        ax.annotate(f"{y}", (x, y), textcoords="offset points", xytext=(dx, dy),
                    ha="center", fontsize=8, color=DISEASE_COLOR[d],
                    bbox=dict(facecolor="white", edgecolor="none", pad=0.4, alpha=0.85))
ax.axvline(0.5, color=C_RED, lw=1.3, ls="--", zorder=2)
ax.text(0.5, max(max(v) for v in tau_counts.values()) * 1.08, "τ=0.5 入选口径", color=C_RED,
        fontsize=8.5, ha="center", va="top",
        bbox=dict(facecolor="white", edgecolor="none", pad=0.5, alpha=0.9))
ax.set_xlabel("入选频率阈值 τ")
ax.set_ylabel("入选稳定特征数")
ax.set_title("τ 敏感性：入选稳定特征数随阈值变化")
...
for ax, d in zip(axes, ["CRC", "IBD"]):
    ...
    labels = [species_name(f) for f in feats]
    masked = np.ma.masked_invalid(M)
    im = ax.imshow(masked, cmap=CMAP_DIVERGING, vmin=-1, vmax=1)
    # NaN 单元格以灰色「N/A」标注
    ax.imshow(np.isnan(M), cmap=LinearSegmentedColormap.from_list("na", ["none", "#e0e0e0"]),
...
fig.suptitle("入选标志物共现 Spearman 相关热图（非零样本、CLR 后丰度；N/A=无法计算）", fontsize=12)
...
ax.set_title("三病稳定标志物 Jaccard 重叠矩阵")
