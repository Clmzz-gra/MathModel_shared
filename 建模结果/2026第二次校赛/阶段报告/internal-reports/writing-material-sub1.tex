...
% VERSION: 2026-08-21-2.4修订（P1-P5 整改：正文去 pkl 字段路径、补登特征系数/置信区间、补图表清单、CLR 增益与 overfit 评估方式裁定）
...
\begin{enumerate}
  ...
  \item 【结果段】核心数字 + 对比结论。要点：主方案下 L2(CLR) AUC 为 Zeller 0.791、metahit 0.887、Chatelier 0.650；RF 在 Zeller/metahit 更优（0.845/0.904）、Chatelier 略优（0.660）；集成不优于单最佳，单最佳（RF）交付；Chatelier 弱信号接近领域下界。表述库参考：3.1-30（数值先导+表格引出）+ 3.2-34（指标比较论证）。
\end{enumerate}
...
0.791/0.611/0.802/0.802 & adenoma 四种方案 L2 AUC & S1-results.pkl adenoma\_sensitivity.*.L2\_AUC & §4.2 \\
0.845/0.651/0.867/0.867 & adenoma 四种方案 RF AUC & S1-results.pkl adenoma\_sensitivity.*.RF\_AUC & §4.2 \\
121/121/95/95 & adenoma 四种方案样本数 & S1-results.pkl adenoma\_sensitivity.*.n\_samples & §4.2 \\
healthy & 默认主方案 & S1-results.pkl adenoma\_sensitivity.selected\_main\_caliber & §4.2 \\
...
S1-adenoma-sensitivity-explore.pdf & small\_adenoma 四种方案敏感性（L2/RF AUC） & adenoma\_sensitivity.*.L2\_AUC / RF\_AUC & §4.2 结果段 \\
...
\begin{itemize}
  \item 可写：主方案（small\_adenoma 归健康）下，L2(CLR) AUC 为 Zeller 0.791、metahit 0.887、Chatelier 0.650；RF 在 Zeller 与 metahit 上更优（0.845、0.904），Chatelier 上略优（0.660）。
...
本子问题涉及 AI 使用记录：[AI-1-1]（领域知识整理与知识检索）、[AI-1-2]（S1 方案设计）、[AI-1-3]（S1 建模实现）、[AI-1-4]（S1 结果分析），见 `.trae/ai-markers/` 与 `handoff-S1-report-agent.md` §5，供阶段 3.3 AI 使用声明引用。
...
\begin{enumerate}
  ...
  \item \textbf{过拟合判定规则失效}（A 级，2.4 已裁定）：三数据集 \texttt{overfit\_flag} 均判 True（全量 AUC=1.0 与 LOOCV 差距 0.196/0.125/0.373 均 $>0.1$），但这是 $n\ll p$ 下样本内 AUC 恒为 1.0 的必然现象。裁定：采用「CV vs LOOCV」评估方式——报告 §3.4 已按此书写（5 折 CV 与 LOOCV 差距 0.014/0.012/0.023，均 $<0.025$，模型稳定），并显式说明 full\_AUC=1.0 背景，不作过拟合判据。
  ...
  \item \textbf{small\_adenoma 主方案选择}：默认(1)（归健康）已落盘；(3)/(4)（剔除）略优但 $\Delta$AUC$<0.05$。报告已按默认(1)书写，最终主方案待建模/本文择优。（B 级）
